...
extern "C" {
int do_fft_compress_vram_friendly(void *const blob, uint16_t const len_t,
                                  uint16_t const len_y, uint16_t const len_x,
                                  uint8_t const len_c, float32_t const fps,
                                  float32_t const freq_limit, void *const dest,
                                  bool use_gpu) {
    ...
    // 1. Initial Load and Padding [cite: 31, 32, 33]
    torch::Tensor tensor = torch::from_blob(
        blob, {len_t, len_y, len_x, len_c}, 
        torch::TensorOptions().dtype(torch::kUint8).device(torch::kCPU)
    ).to(device).to(torch::kFloat32);

    // Padding logic [cite: 32, 33]
    tensor = do_pad_video(tensor); 
    
    // 2. Sequential FFT Pass 1: Time (Dimension 0)
    // We use rfft here because the input is real.
    // Result becomes complex. Shape: [len_t/2 + 1, H, W, C]
    tensor = torch::fft::rfft(tensor, /*n=*/std::nullopt, /*dim=*/0);

    // 3. Immediate Temporal Truncation
    // Calculate cutoff 'n' based on freq_limit [cite: 37, 38, 39]
    int64_t n_cutoff = torch::sum(
        (torch::fft::rfftfreq(len_t, torch::TensorOptions().device(torch::kCPU)) * fps) < freq_limit
    ).item().toLong();
    
    tensor = tensor.narrow(0, 0, n_cutoff); // Reduce VRAM footprint early 

    // 4. Sequential FFT Pass 2: Height (Dimension 1)
    // Must use complex fft now.
    tensor = torch::fft::fft(tensor, std::nullopt, /*dim=*/1);

    // 5. Sequential FFT Pass 3: Width (Dimension 2)
    tensor = torch::fft::fft(tensor, std::nullopt, /*dim=*/2);

    // 6. Post-FFT Processing (Shift and Spatial Truncation)
    // Shift spatial dims (now dims 1 and 2) [cite: 42]
    tensor = torch::fft::fftshift(tensor, {1, 2});

    uint16_t len_max = std::max(len_x, len_y);
    uint16_t len_truncated = len_max >> 3; [cite: 43]
    uint16_t pos_start = (len_max - len_truncated) >> 1; [cite: 43]
    uint16_t pos_end = pos_start + len_truncated; [cite: 44]

    // Slice spatial dimensions [cite: 44]
    tensor = tensor.index({
        torch::indexing::Slice(), 
        torch::indexing::Slice(pos_start, pos_end),
        torch::indexing::Slice(pos_start, pos_end),
        torch::indexing::Slice()
    });

    // 7. Magnitude/Phase Concatenation and Interpolation [cite: 45, 46]
    // Permute to [C, H, W, T] for interpolation compatibility
    tensor = tensor.permute({3, 1, 2, 0}); 
    
    torch::Tensor compressed = torch::nn::functional::interpolate(
        torch::cat({tensor.abs(), tensor.angle()}, 0).unsqueeze(0),
        torch::nn::functional::InterpolateFuncOptions()
            .size(std::vector<int64_t>({len_truncated, len_truncated, 60}))
            .mode(torch::kTrilinear)
            .align_corners(false)
    ).squeeze().to(torch::kCPU).contiguous(); [cite: 46, 47]

    std::memcpy(dest, compressed.data_ptr(), compressed.nbytes());
    return 0;
}
}
...
#+begin_src sh :shebang #!/bin/sh :results output :mkdirp yes :tangle ./.git.sh
  ...
  git status ; git commit -m 'Added more docker scripts'
