...
***** COMMENT Define macro to indicate running on nvidia gpu
#+begin_src cpp :mkdirp yes :tangle ./src/main.hpp
  #define USE_CUDA 1
#+end_src
...
** Commit and push
#+begin_src sh :shebang #!/bin/sh :results output :mkdirp yes :tangle ./.git.sh
  git status ; git commit -m 'Added a check to ensure repetedly getting the same file does not cause a race condition'
