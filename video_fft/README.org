...
* Implementing locks related
#+begin_src rust :mkdirp yes :tangle ./src/lockssync.rs
  use crate::export;
...
#+begin_src rust :mkdirp yes :tangle ./src/videofn.rs
  ...
  pub fn convert_encoded_video_to_raw_outpipe(
      ...
      let mut child = std::process::Command::new("ffmpeg")
          .args([
              "-i", path_file_video_input,
              "-vf", format!("fps={},scale={}:{}", fps, size_x, size_y).as_str(),
              "-nostdin", "-f", "rawvideo",
              "-pix_fmt", "rgb24",
              "pipe:1",
          ])
...
#+begin_src rust :mkdirp yes :tangle ./src/videofft.rs
  ...
  fn get_num_windows(total_video_length: u64) -> u8 {
      ...
      if total_video_length < (min_length as u64) {
          ...
          let float_val = (((total_video_length - (ideal_length as u64)) as f64)
                          / (stride as f64) ) as f64;
  ...
  impl fft_video {
      ...
      pub fn from_torch_video_tensor(
          ...
          if true {
              ...
              if status != 0 {
                  ...
                  return Err(anyhow::format_err!("FFT step failed with exit status {}...", status));
              }
          }
      ...
      pub fn windowed_from_torch_video_tensor(
          ...
          let num_windows: u8 = get_num_windows(/*total_video_length: u64 =*/ total_video_length as u64);
          ...
          match num_windows {
              ...
              1 => {
                  eprintln!("window_length = 1, total_video_length = {}",total_video_length);
              ...
              _ => {
                  let mut list_torch_video_tensor = Vec::<tch::Tensor>::with_capacity(num_windows as usize);

                  for i in 1..=num_windows {
                      let end = ((((total_video_length - 160) * ((i as i64) - 1))
                          / ((num_windows as i64) - 1))
                          + 160).min(total_video_length);

                      let start = (end - 160).max(0);

                      eprintln!("i = {} , num_windows = {} , total_video_length = {}",
...
( T * x ) / 15 for choosing images for inference
...
#+begin_src rust :mkdirp yes :tangle ./src/serverinferencechannel.rs
  ...
  impl inference_pair {
      ...
      pub fn do_infer_on_video_file_and_get_video_tensor(
          ...
          self.slave_sender
              .do_infer_on_video_file_and_get_video_tensor(/*path_file_video_input: &str =*/ path_file_video_input)
      }
  }
#+end_src
...
#+begin_src rust :mkdirp yes :tangle ./src/serverinferencechannelboth.rs
  ...
  impl combined_infer {
      ...
      pub fn do_infer_on_video_file(
          ...
          let image_batch_tensor: tch::Tensor = {
              const end_index: u8 = (serverinferencechannelimage::GOOD_BATCH_SIZE * NUM_BATCHES_IN_VIDEO);
              let image_indices: std::vec::Vec<i64> = (0..end_index)
                  .map(|x: u8|
                       ((video_tensor.size()[0] - 1) * (x as i64)) /
                       ((end_index - 1) as i64)
                  ).collect();
...
** Commit and push
#+begin_src sh :shebang #!/bin/sh :results output :mkdirp yes :tangle ./.git.sh
  git status ; git commit -m 'Fully working elegant RD + image inference code'
