...
#+begin_src rust :mkdirp yes :tangle ./src/server.rs
  ...
  impl inference_slave {
      ...
      fn efficient_infer(vals: &mut Vec<videofft::fft_video>) -> anyhow::Result<Vec<inferencerelated::infer_results>> {
          let mut infer_slave = inferencerelated::infer_slave::new(1);
      ...
      pub fn inference_loop(&self) -> anyhow::Result<()> {
          loop {
              ...
              if true {
                  let ret = Self::efficient_infer(/*vals: &mut Vec<videofft::fft_video> =*/&mut tensors);
