...
1. Check acquiring lock happens before constructing torch AOT inference class.
2. It is released only after the destructor.
...
***** Inference class for image cv usability
...
#+begin_src rust :mkdirp yes :tangle ./src/export.rs
  unsafe extern "C" {
      pub fn new_infer_slave_image_cv_usability(batch_size: ::std::os::raw::c_uchar) -> *mut ::std::os::raw::c_void;
...
** CV USability
#+begin_src rust :mkdirp yes :tangle ./src/inferencerelatedimagecvusability.rs
  use crate::export;
  use tch::IndexOp;
...
* Implement the inference loop part for cv usability
#+begin_src rust :mkdirp yes :tangle ./src/serverinferencechannelimagecvusability.rs
...
#+begin_src sh :shebang #!/bin/sh :results output :mkdirp yes :tangle ./.git.sh
  ...
  GITADD 'src/inferencerelatedimagecvusability.rs'
  ...
  GITADD 'src/serverinferencechannelimagecvusability.rs'
...
** Commit and push
#+begin_src sh :shebang #!/bin/sh :results output :mkdirp yes :tangle ./.git.sh
  git status ; git commit -m 'Mostly completed implementing the inference class for cv usability'
