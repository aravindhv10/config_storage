  ...
  impl infer::rdvideoinfer_server::Rdvideoinfer for grpc_inferer {
      async fn doinfer(
          ...
          let res = tokio::task::spawn_blocking(move || {
              infpair.do_infer_on_video_file(&path)
          }).await.expect("The blocking task panicked");
          ...
          match res {
              Ok(o) => {
                  let preds: Vec<infer::Grpcvideoprediction> = o
                      ...
                      .collect();
                  let avg = o.iter().map(|i| i.majority() as f32).sum::<f32>() /
                            (o.len() as f32) ;
...
** Commit and push
#+begin_src sh :shebang #!/bin/sh :results output :mkdirp yes :tangle ./.git.sh
  git status ; git commit -m 'Fixed division by 0 issue'
