  ...
  impl video_slicer {
      pub fn new(
          ...
          if path_file_rawvideo_output.is_none() {
              ...
              let tmppath = std::path::PathBuf::from(format!("/dev/shm/{:x}.{:x}.raw", name_hash, hash));
...
#+begin_src rust :mkdirp yes :tangle ./src/serverinferencechannel.rs
  ...
  impl inference_pair {
      ...
      pub fn do_infer_on_video_file(
          ...
          self.slave_sender.do_infer_on_video_file(path_file_video_input)
      }
  }
#+end_src
...
** Commit and push
#+begin_src sh :shebang #!/bin/sh :results output :mkdirp yes :tangle ./.git.sh
  git status ; git commit -m 'Regular updates with logging and code cleanups'
