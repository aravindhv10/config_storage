...
#+begin_src conf :mkdirp yes :tangle ./Cargo.toml
  fjall = { version = "3.1.5", features = ["lz4", "default"] }
  leapfrog = { version = "0.3", features = ["serde", "stable_alloc"] }
  memmap2 = "0.9"
  redb = "4"
#+end_src
...
****** COMMENT Zero copy and type casting
#+begin_src conf :mkdirp yes :tangle ./Cargo.toml
...
* Key - Value database
#+begin_src rust :mkdirp yes :tangle ./src/keyvaluedb.rs
  use prost::Message;
  ...
  impl inference_communicator {
      ...
      fn do_infer_on_fft_tensor(
          ...
          match self.sender.send(msg) {
              ...
              Err(_) => {
                  tracing::error!("Failed to send inference request to the serving thread");
                  return Err(anyhow::format_err!(
                      "Failed to send inference request to the serving thread"
                  ));
              }
      ...
      async fn do_infer_on_fft_tensor_async(
          ...
          match self.sender.send(msg) {
              Ok(_) => {
                  tracing::info!("Successfully sent inference request to the serving thread");
              }
              Err(_) => {
                  tracing::error!("Failed to send inference request to the serving thread");
                  return Err(anyhow::format_err!(
          ...
          loop {
              ...
              if do_infer {
                  ...
                  for (i, j) in ret.into_iter().zip(senders.into_iter()) {
                      match j.send(i) {
                          Ok(_) => {
                              tracing::info!("sending inference results back");
                          }
                          Err(_) => {
                              tracing::info!("Failed to send back inference results");
                          }
                      };
                  }
              }
      ...
      async fn doinfer(
          ...
          match res {
              ...
              Err(e) => {
                  ...
                  return Err(tonic::Status::internal(
                      format!("Internal error, inference failed due to {}", e)
                  ));
              }
          }
      }
...
** Commit and push
#+begin_src sh :shebang #!/bin/sh :results output :mkdirp yes :tangle ./.git.sh
  git status ; git commit -m 'Fixed tonic return error codes'
