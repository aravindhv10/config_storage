...
type message_output = Vec<inferencerelatedimage::infer_results>;
...
fn get_message_output_pair() -> (message_output_sender, message_output_receiver) {
    ...
    // tokio::sync::oneshot::channel::<message_output>()
}
            ...
            for (x, y) in res.into_iter().zip(senders.into_iter()) {
                match x {
                    Ok(o) => {
                        ...
                        // match y.send(o) {
                        //     Ok(_) => {
                        //         tracing::info!("Sent inference results.");
                        //     }
                        //     Err(_) => {
                        //         tracing::error!("Failed to send inference results.");
                        //     }
                        // };
                    }
                    Err(e) => {
